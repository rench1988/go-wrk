...
func StartClient(url_, heads, requestBody string, meth string, dka bool, responseChan chan *Response, waitGroup *sync.WaitGroup, tc int, proxyUrl string) {
	...
	if err == nil && u.Scheme == "https" {
		...
		if proxyUrl != "" {
			...
			tr = &http.Transport{TLSClientConfig: tlsConfig, DisableKeepAlives: dka, Proxy: http.ProxyURL(u)}
		} else {
		...
		if proxyUrl != "" {
			u, _ := url.Parse(proxyUrl)
			tr = &http.Transport{DisableKeepAlives: dka, Proxy: http.ProxyURL(u)}
		} else {
